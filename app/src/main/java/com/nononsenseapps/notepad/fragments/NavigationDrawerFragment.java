...
abstract class NavigationDrawerFragment_USELESS extends Fragment{
	...
	private class ExtraHeaderItemViewHolder extends RecyclerView.ViewHolder implements View
		...
		public ExtraHeaderItemViewHolder(View itemView) {
			...
			mAvatar = (ImageView) itemView.findViewById(0);//R.id.item_avatar);
		}
	...
	private class CursorViewHolder extends RecyclerView.ViewHolder implements View
		...
		public void bind(Cursor cursor) {
			final int colTitle = cursor.getColumnIndex(TaskList.Columns.TITLE);
			final int colId = cursor.getColumnIndex(TaskList.Columns._ID);
			final int colViCount = cursor.getColumnIndex(TaskList.Columns.VIEW_COUNT);

			final String title = cursor.getString(colTitle);
			id = cursor.getLong(colId);
			mTitle.setText(title);
			mCount.setText(cursor.getString(colViCount));

			TextDrawable drawable = TextDrawable.builder().buildRound(
					title.toUpperCase().substring(0, 1),
					ColorGenerator.Companion.getMATERIAL().getColor(title));
