...
public class DBProviderMovementTest  extends AndroidTestCase  {
	...
	private void assertTaskLeftRightAreSequential(final long listId) {
		...
		assertEquals("Must be duplicate positions in the list", 0, positions.size());
	}
	...
	private ArrayList<Task> getTasks(final long listId) {
		...
		final Cursor c = resolver.query(Task.URI_INDENTED_QUERY, Task.Columns.FIELDS,
				null, new String[] { Long.toString(listId) }, null);
		assertCursorGood(c);
		while (c.moveToNext()) {
	...
	private ArrayList<Task> getDeletedTask(final String title) {
		final ArrayList<Task> results = new ArrayList<Task>();
		final Cursor c = resolver.query(Task.URI_DELETED_QUERY, Task.Columns.FIELDS,
				Task.Columns.TITLE + "IS ?", new String[] { title }, null);
		assertCursorGood(c);
		while (c.moveToNext()) {
			results.add(new Task(c));
		}
	...
	private ArrayList<Task> moveAndAssert(final TaskList tl, final int fromPos,
			final int toPos) {
		Log.i("nononsenseapps test", "Testing move from: " + fromPos + " to " + toPos);
		// Get ordered
	...
	private ArrayList<Task> indentAndAssert(final Task orgTask,
		...
		//resolver.update(orgTask.getUri(), orgTask.getIndentValues(), null, null);
		...
		for (int i = 0; i < orgTasks.size(); i++) {
			...
			if (expectedSuccess && resultTasks.get(i)._id == orgTask._id) {
				...
				assertEquals("Indent should increase level", orgTask.level + 1, resultTasks.get(i).level);
				...
				if (resultTasks.get(i)._id == orgTask._id) {
					assertEquals("Failed indent should not increase level", orgTask.level, resultTasks.get(i).level);
				}
	...
	private ArrayList<Task> unIndentAndAssert(final Task orgTask,
		...
		assertTrue("You can not expect a success if the task is a root!", expectedSuccess == (orgTask.level > 1));
		...
		for (int i = 0; i < orgTasks.size(); i++) {
			...
			if (expectedSuccess && resultTasks.get(i)._id == orgTask._id) {
				assertTrue("UnIndent should mean left = preleft + 1",
						orgTask.left + 1 == resultTasks.get(i).left);
				assertEquals("Indent should decrease level", orgTask.level - 1, resultTasks.get(i).level);
			} else if (!expectedSuccess) {
				...
				if (resultTasks.get(i)._id == orgTask._id) {
					assertEquals("Failed unindent should not change level", orgTask.level, resultTasks.get(i).level);
				}
			} 
		}
	...
	public void testIndents() {
		...
		assertEquals("Task level should be two", 2, orgTasks.get(count - 1).level);
		...
		for (int i = orgTasks.size(); i > 1; i--) {
			assertEquals("Level incorrect (i = " + i + ")", i, orgTasks.get(orgTasks.size() - 1).level);
			orgTasks = unIndentAndAssert(orgTasks.get(orgTasks.size() - 1), true);
		}
		// Should now be a root
		assertEquals("Level incorrect", 1, orgTasks.get(orgTasks.size() - 1).level);
		...
		for (int j = 1; j < orgTasks.size() - 1; j++) {
			for (int i = j + 1; i - cum > 1; i--) {
				assertEquals("Level incorrect (i = " + i + ")", i - cum, orgTasks.get(j).level);
	...
	public void testInvalidIndents() {
		// TODO
		// fail("TODO");
	}
	...
	public void testIndentedUri() {
		...
		failed = false;
		try {
			c = resolver.query(Task.URI_INDENTED_QUERY, Task.Columns.FIELDS, null,
					null, null);
		} catch (SQLException e) {
			failed = true;
		}
