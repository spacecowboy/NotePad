...
/**
 * An object that represents the task information contained in
 * the database. Provides convenience methods for moving and
 * indenting items.
 */
public class Task extends DAO {
	...
	public static void addMatcherUris(UriMatcher sURIMatcher) {
		...
		sURIMatcher.addURI(MyContentProvider.AUTHORITY, TABLE_NAME + "/" + INDENTEDQUERY, INDENTEDQUERYCODE);
		sURIMatcher.addURI(MyContentProvider.AUTHORITY, TABLE_NAME + "/" + MOVESUBTREE + "/#", MOVESUBTREECODE);
		
		sURIMatcher.addURI(MyContentProvider.AUTHORITY, TABLE_NAME + "/" + INDENT, INDENTCODE);
		sURIMatcher.addURI(MyContentProvider.AUTHORITY, TABLE_NAME + "/" + INDENT + "/#", INDENTITEMCODE);
		sURIMatcher.addURI(MyContentProvider.AUTHORITY, TABLE_NAME + "/" + UNINDENT, UNINDENTCODE);
	...
	// Special URI to look at backup table
		public static final Uri URI_DELETED_QUERY = Uri.withAppendedPath(
				URI, DELETEDQUERY);
	
	// Special URI where the last column will be a count
	public static final Uri URI_INDENTED_QUERY = Uri.withAppendedPath(
			URI, INDENTEDQUERY);
	
	// Special URI to use when a move is requested
	public static final Uri URI_WRITE_MOVESUBTREE = Uri.withAppendedPath(
			URI, MOVESUBTREE);
	...
	public static final Uri URI_WRITE_INDENT = Uri.withAppendedPath(
			URI, INDENT);
	...
	public static final Uri URI_WRITE_UNINDENT = Uri.withAppendedPath(
			URI, UNINDENT);
	...
	public static class Columns implements BaseColumns {
		...
		public static final String[] FIELDS = { _ID, TITLE, NOTE, COMPLETED, DUE, UPDATED,
				LEFT, RIGHT, DBLIST,
				GTASKACCOUNT, GTASKID,
				DROPBOXACCOUNT, DROPBOXID};
		public static final String[] DELETEFIELDS = { _ID, TITLE, NOTE, COMPLETED, DUE, DBLIST };
		// Same but no ID
	...
	public static final String CREATE_TABLE = "CREATE TABLE " + TABLE_NAME
			...
			+ " INTEGER NOT NULL DEFAULT 2," + Columns.DBLIST
			+ " INTEGER NOT NULL,"

			// Positions must be positive and ordered!
			+ " CHECK(" + Columns.LEFT + " > 0), "
			+ " CHECK(" + Columns.RIGHT + " > 0), "
	...
	public static final String CREATE_DELETE_TABLE = "CREATE TABLE " + DELETE_TABLE_NAME
			+ "(" + Columns._ID + " INTEGER PRIMARY KEY," + Columns.TITLE
			+ " TEXT NOT NULL DEFAULT ''," 
			+ Columns.NOTE
			+ " TEXT NOT NULL DEFAULT ''," 
			+ Columns.COMPLETED + " TEXT,"
			+ Columns.DUE + " TEXT,"
			+ Columns.DBLIST + " INTEGER)";
	 ...
	 */
	public ContentValues getMoveValues(final long targetLeft, final long targetRight) {
	...
	/**
	 * An indent operation should be performed alone. No other information should
	 * accompany such an update.
	 */
	...
	public static final String TRIGGER_PRE_DELETE = String.format(
					...
					+ " INSERT INTO %2$s (" + arrayToCommaString("", Columns.DELETEFIELDS_TRIGGER, "") + ") "
					+ " VALUES(" + arrayToCommaString("old.", Columns.DELETEFIELDS_TRIGGER, "") + "); "
							
			+ " END;", TABLE_NAME, DELETE_TABLE_NAME);

	public static String getSQLIndentedQuery(final String[] fields) {
		return String.format("SELECT " + arrayToCommaString("T2.", fields, "") + ", COUNT(T1.%4$s) AS %5$s " + 
				" FROM %1$s AS T1, %1$s AS T2 " + 
				...
				TABLE_NAME, Columns.LEFT, Columns.RIGHT, Columns._ID, INDENT, Columns.DBLIST);
	}
	
	public boolean shouldMove(final ContentValues values) {
		return values.containsKey(TARGETLEFT) && values.containsKey(TARGETRIGHT) &&
				left != null && right != null && dblist != null 
				&& values.getAsLong(TARGETLEFT) > 0 
				&& values.getAsLong(TARGETRIGHT) > values.getAsLong(TARGETLEFT);
	}
	
	public int move(final ContentResolver resolver, final long targetLeft, final long targetRight) {
		return resolver.update(getMoveSubTreeUri(), getMoveValues(targetLeft, targetRight), null, null);
	}
	...
	public static int indentMany(final ContentResolver resolver, final long[] idList) {
		if (idList.length < 1) {
		...
		return resolver.update(URI_WRITE_INDENT, new ContentValues(), whereClause, null);
	}
	
	public int unindent(final ContentResolver resolver) {
		return resolver.update(getUnIndentUri(), getIndentValues(), null, null);
	}
	public static int unIndentMany(final ContentResolver resolver, final long[] idList) {
		if (idList.length < 1) {
			throw new InvalidParameterException("Must give some ids to indent");
		}
		// Ex: WHERE _id IN (15,27,2,94)
		...
		whereClause = whereClause.substring(0, whereClause.length() - 1) + ")";
		return resolver.update(URI_WRITE_UNINDENT, new ContentValues(), whereClause, null);
	}
	
	@SuppressLint("DefaultLocale")
	public String getSQLUnIndentItem(final long parentRight) {
		//final String GETPARENTRIGHT = "SELECT %3$s FROM %1$s WHERE %2$s < %4$s AND %3$s > %5$s AND %6$s IS %7$d ORDER BY (%3$s - %2$s) ASC LIMIT 1";
		if (shouldIndent())
			// First left value
			return String.format("UPDATE %1$s SET %2$s = " +
				...
				// Restrict to list
			 	...
			 	left, right, Columns.DBLIST, dblist
				);
