...
//		for (long id: taskIds) {
//			final Cursor c = getActivity().getContentResolver().query(Task.getUri(id), Task.Columns.FIELDS, null, null, null);
//			
//			if (c.moveToFirst()) {
//				Task t = new Task(c);
...
//		}
		...
		final String whereId = new StringBuilder(Task.Columns._ID).append(" IN (")
				.append(DAO.arrayToCommaString(taskIds)).append(")").toString();
	...
	void okClicked() {
		...
		final Cursor c = (Cursor) listView.getItemAtPosition(listView.getCheckedItemPosition());
		if (c != null) {
